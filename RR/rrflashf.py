...
def decompress_and_flash_rom(archive_path):
    RF = "/sdcard/Download/mi-flash-fastboot-rom"
    if not os.path.exists(RF):
        os.makedirs(RF)
    print("\n\033[92mDecompressing..., please wait\033[0m\n")

    if archive_path.endswith(".tgz"):
        tar_command = f"pv -bpe '{archive_path}' | tar --strip-components=1 -xzf - -C '{RF}/' > /dev/null 2>&1"
        return_code = os.system(tar_command)
    else:
        cmd = f"7z x -y -bso0 -bsp1 -o'{RF}/' '{archive_path}'"
    ...
    print("\n\033[92mExtraction finished successfully!\033[0m\n")
    ...
    if os.path.exists(os.path.join(RF, "images")) and any(os.path.exists(os.path.join(RF, file)) for file in ["flash_all_lock.sh", "flash_all.sh", "flash_all_except_data_storage.sh"]):
        ...
        for root, dirs, files in os.walk(RF):
            if any(f in files for f in ["flash_all.sh", "flash_all_lock.sh"]):
                flash_selected_result(root)
                return
        print("\ninvalid archive 'exit'\n")
        exit()

target_extensions = (".tgz", ".7z", ".rar", ".zip")
...
for root, dirs, files in os.walk("/sdcard"):
    ...
    matched_files = [f for f in files if f.lower().endswith(target_extensions)]
    result_paths.extend([os.path.join(root, f) for f in matched_files])

    for dir_name in dirs:
        dir_path = os.path.join(root, dir_name)
        try:
            dir_files = set(os.listdir(dir_path))
            if any(target_file in dir_files for target_file in target_files) and target_folder in dir_files:
...
if result_paths:
    ...
    if selected_result.lower().endswith(target_extensions):
